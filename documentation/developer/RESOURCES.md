# Developer Resources

## Resources that may help with design decisions in OCTproEngine:

### Online
- **Bien's Space: API Design for C++**  
  https://biendltb.github.io/book/api-design-for-cpp-martin-reddy-part-1/
  https://biendltb.github.io/book/api-design-for-cpp-martin-reddy-part-2/

- **KDE Wiki: Binary Compatibility of C++ libraries**  
  https://community.kde.org/Policies/Binary_Compatibility_Issues_With_C++
  
- **Deb Haldar: Top 25 C++ API design mistakes**
  https://acodersjourney.com/top-25-cplusplus-api-design-mistakes-and-how-to-avoid-them/

- **Microsoft: Modern C++ best practices for exceptions and error handling**
  https://learn.microsoft.com/en-us/cpp/cpp/errors-and-exception-handling-modern-cpp

## Books
- **API Design for C++ - Martin Reddy** (for summary see Bien's Space and/or Deb Haldar's notes)

&nbsp;

&nbsp;


## Resources that may help with the basics:

## Oline
- **James Akl: CUDA ontology**
  https://jamesakl.com/posts/cuda-ontology/
