...
def test_basic_multi_consumer():
    ...
    # Add 3 callbacks
    id1 = processor.add_output_callback(lambda data, buffer_id: counts.update({'cb1': counts['cb1'] + 1}))
    id2 = processor.add_output_callback(lambda data, buffer_id: counts.update({'cb2': counts['cb2'] + 1}))
    id3 = processor.add_output_callback(lambda data, buffer_id: counts.update({'cb3': counts['cb3'] + 1}))

    # Verify count
    assert processor.get_output_callback_count() == 3, "Should have 3 callbacks"
...
def test_remove_callback():
    ...
    assert processor.get_output_callback_count() == 3
    ...
    assert processor.get_output_callback_count() == 2
...
def test_clear_callbacks():
    ...
    processor.add_output_callback(lambda data, buffer_id: counts.update({'cb1': counts['cb1'] + 1}))
    processor.add_output_callback(lambda data, buffer_id: counts.update({'cb2': counts['cb2'] + 1}))
    ...
    assert processor.get_output_callback_count() == 0
...
def test_legacy_api():
    ...
    # Use legacy API 
    processor.set_callback(lambda data, buffer_id: count.__setitem__(0, count[0] + 1))

    # Legacy set_callback should add one output callback
    assert processor.get_output_callback_count() == 1
